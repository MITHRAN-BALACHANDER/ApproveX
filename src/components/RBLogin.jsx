...
function RBLogin({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/role-auth/login', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        {/* Login Form */}
        <div className={`${currentConfig.bgColor} ${currentConfig.borderColor} border-2 rounded-xl shadow-xl p-8`}>
          <div className="text-center mb-6">
            <div className={`mx-auto h-16 w-16 bg-gradient-to-r ${currentConfig.color} rounded-full flex items-center justify-center text-2xl mb-4`}>
              {currentConfig.icon}
            </div>
            <h3 className="text-2xl font-bold text-gray-900">{currentConfig.title}</h3>
        ...
        <div className="text-center text-sm text-gray-600">
          <div className="space-y-2">
            {selectedRole === 'student' && (
              <p>Use your college email and password to access your OD requests</p>
            )}
            {selectedRole === 'teacher' && (
              <p>Use your teacher credentials to review and approve OD requests</p>
            )}
            {selectedRole === 'admin' && (
